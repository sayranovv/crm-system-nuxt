...
const { mutate, isPending } = useMutation({
  ...
  mutationFn: async (data: IDealFormState) => {
    try {
      await DB.createDocument(DB_ID, COLLECTION_DEALS, uuid(), data);
    } catch (error) {
      console.error("Ошибка при создании сделки:", error);
      throw error; // Повторное выбрасывание для обработки в onError при необходимости
    }
  },
  onSuccess() {
  ...
  onError(error) {
    alert("Не удалось создать сделку: " + error.message); // Обратная связь для пользователя
  }
});
...
const onSubmit = handleSubmit(values => {
  const formattedValues = {
    ...values,
    price: parseInt(values.price, 10), // Преобразование в целое число
  };
